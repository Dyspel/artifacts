...
| Feature                                                          | Status |
| ---------------------------------------------------------------- | ------ |
| Chunked-KV / object-store `Storage` impl — `ObjectStore` trait (read + write) + `MemObjectStore` + atomic-write `FsObjectStore` + conformance suite landed; production routing + chunked-KV impl remain | 🟡 M2b |
...
1. **M2b — chunked-KV `Storage` impl.** `ObjectStore` trait
   (`read_loose` + `write_loose`) + two impls — `FsObjectStore`
   with atomic tmp+rename writes, `MemObjectStore` — and a
   shared conformance suite landed. The trait shape is no longer
   FS-specific; a chunked-KV impl plugs in by satisfying the same
   conformance helpers. Production code doesn't route through the
   trait yet (reads still touch the filesystem directly or via
   gix); routing the receive-pack and commits-plumbing paths
   through `ObjectStore` is the next slice, then the chunked-KV
   impl itself.
...
# Test
cargo test                  # 213 unit tests (storage, smart-http, refs, commits, tokens, auth, jwt, ownership, rate-limit, request-id, audit, gc, webhooks, config rotation, audit log + retention, webhook-secret encryption + master-key rotation, object-store read+write conformance, bind-safety)
...
| **M2b**     | 🟡 | second `Storage` impl — objects chunked into a KV, matching the DO+SQLite shape. `ObjectStore` trait (`read_loose` + `write_loose`) + atomic-write `FsObjectStore` + `MemObjectStore` + a shared conformance suite landed (proof the trait isn't FS-specific, and that both impls round-trip writes byte-exact). Production read/write routing + the chunked-KV impl itself + lifecycle ops remain. | bare repos on disk |
