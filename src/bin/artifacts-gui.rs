    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            match self.view {
                ...
                View::Forks => {
                    if let Some(clicked) = render_forks(
                        ui,
                        &snapshot,
...
///
/// Layout is a classic tree layout: leaves get consecutive x coordinates,
/// internal nodes sit at the midpoint of their first and last child.
/// Depth maps to y. Multiple roots lay out left-to-right.
///
/// Interaction:
///   - Drag to pan
///   - Scroll to zoom (centered on cursor)
///   - Click a node to select → Detail tab
///
/// Pan/zoom state lives on `App` so it persists across frames and tab
/// switches. Zoom is clamped to a sensible range.
fn render_forks(
    ui: &mut egui::Ui,
    s: &StateSnapshot,
    selected: Option<&str>,
    ...
    let base_origin = egui::pos2(
        rect.left() + cell_w * 0.5,
        rect.top() + cell_h * 0.5,
    );
    let to_px = |(x, y): (f64, f64)| -> egui::Pos2 {
        let p = base_origin
            + egui::vec2(x as f32 * cell_w, y as f32 * cell_h) * *zoom
            + *pan;
        p
    };
    ...
    for r in &s.repos {
        ...
        let (Some(child_pos), Some(parent_pos)) =
            (positions.get(&r.id), positions.get(parent))
        else {
            continue;
        };
    ...
    for r in &s.repos {
        let Some(pos) = positions.get(&r.id) else { continue };
        ...
        // Node text: short-id first line, owner second. Use two paint
        // calls so the owner line is muted.
...
";
    ...
    fn layout_subtree_parent_centers_over_children() {
        // root has 3 children at depth 1. Children should land at x=0, 1, 2;
        // root should land at x=1 (midpoint of first and last).
        let root = sm("r", None);
        let c1 = sm("c1", Some("r"));
        let c2 = sm("c2", Some("r"));
        let c3 = sm("c3", Some("r"));
    ...
    fn layout_subtree_nested() {
        // r -> [a, b]; a -> [a1, a2]; b is a leaf.
        // Expected leaves in order: a1(0), a2(1), b(2).
        // a centers over [a1, a2] = 0.5
        // r centers over [a, b] = (0.5 + 2.0) / 2.0 = 1.25
        let r = sm("r", None);
        let a = sm("a", Some("r"));
        let a1 = sm("a1", Some("a"));
        let a2 = sm("a2", Some("a"));
